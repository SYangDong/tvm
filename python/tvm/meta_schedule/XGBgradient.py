...
class XGBGradientSearchTuner:
    ...
    def __init__(
        ...
        if not has_sample_instruction(self.design_spaces):
            self.num_trials_per_iter = self.max_trials = len(self.design_spaces)
        else:
    ...
    def XGB_gradient_search(self):
        ...
        if self.max_trials < 64:
            tuning_records = self.get_sample_records(self.max_trials)
