...
def test_dynamic_gradient_descent():
    ...
    max_tuning_time = (
        360  # Maximum tuning time in seconds, 120 is the suggested value
    )
    ...
    if record is not None:
        ...
        print(
            f"Evaluator time:  {evaluator(buff_a, buff_b, buff_c).mean * 1000} ms"
        )
    else:
        print("[INFO]*************Failed!")
...
def test_meta_schedule():
    ...
    search_time /= 60
    print(
        f"Meta schedule total search time: {search_time} minutes", flush=True
    )
    record = database.query_tuning_record(
        _normalize_mod(matmul),
        Target("llvm -mcpu=icelake-server -num-cores 28"),
    ...
    if record is not None:
        ...
        print(
            f"meta schedule Evaluator time:  {evaluator(buff_a, buff_b, buff_c).mean * 1000} ms"
        )
    else:
        print("[INFO]*************Failed!")
